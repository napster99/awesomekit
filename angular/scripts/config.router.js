 ...
 */
 ...
 //https://www.getpostman.com/collections/9ca15c198093fddc35a3
(function() {
    'use strict';
    angular
      .module('app')
      .run(runBlock)
      .constant('ROOT_URL' , 'http://api.dashboard.com/')
      .config(config)
      ...
      .factory('httpInterceptor', [ '$q', '$injector',function($q, $injector) { 
          var httpInterceptor = { 
            ...
            'response' : function(response) { 
              var rootScope = $injector.get('$rootScope'); 
              if(response.data.errcode == 3001) {
                // return rootScope.$state.go('access.signin'); 
              }
              return response; 
            } 
          } 
        ...
        $stateProvider
            .state('app', {
              ...
              views: {
                '': {
                  templateUrl: layout,
                  controller : 'MyNavCtrl',
                  resolve: load(['scripts/controllers/myNavCtrl.js'])
                }
              }
            })
            .state('app.dashboard', {
            ...
            .state('app.robot', {
              url: '/robot',
              templateUrl: 'apps/robot.html',
              data : { title: 'xxxxxx' },
              controller: "RobotCtrl",
              resolve: load(['apps/mylibs/jquery.ztree.core.min.js', 'ui.select','apps/mylibs/jquery.ztree.excheck.js', 'apps/mylibs/page.js','apps/robot.js'])
            })

            //xxxxx
            .state('app.topicPackage', {
              url: '/topicPackage',
              templateUrl: 'apps/topicPackage.html',
              data : { title: 'xxxxxx' },
              controller: "TopicPackageCtrl",
              resolve: load(['apps/mylibs/page.js','ui.select','apps/topicPackage.js'])
            })

            
            // xxxx
            .state('app.officialMessage', {
              url: '/officialMessage',
              template: '<div ui-view></div>'
            })
            .state('app.officialMessage.public', {
              url: '/public',
              templateUrl: 'apps/officialMessage/publicMessage.html',
              data : { title: 'xxxxx' },
              resolve: load(['ui.select', 'apps/mylibs/page.js','apps/officialMessage/publicMessage.js'])
            })
            .state('app.officialMessage.private', {
              url: '/private',
              templateUrl: 'apps/officialMessage/privateMessage.html',
              data : { title: 'xxxxxxx' },
              resolve: load(['ui.select', 'ui.bootstrap','apps/officialMessage/privateMessage.js'])
            })
